...
# Example implementation of an OpenAI powered API endpoint
# This endpoint is provided as an example of how to implement an external API endpoint
# To test this endpoint, you can use the following curl command:
# curl -X POST http://localhost:50505/analyze_sentiment_example -H "Content-Type: application/json" -d '{"text": "I am really excited about this new AI feature!"}'
#
# Authenticaed requests are also supported:
# curl -X POST http://localhost:5000/analyze_sentiment -H "Content-Type: application/json" -H "Authorization: Bearer YOUR_TOKEN" -d '{"text": "I am really excited about this new AI feature!"}'
...
# current_app.logger.info("Example API auth bearer token %s", token.token)

@bp.route("/analyze_sentiment_example", methods=["POST"])
@authenticated
async def analyze_sentiment_example(auth_claims: Dict[str, Any]):
    if not request.is_json:
        return jsonify({"error": "request must be json"}), 415
    ...
    try:
        ...
        messages = [
            {"role": "system", "content": "You are a sentiment analysis expert. Analyze the following text and provide sentiment score from -1.0 (most negative) to 1.0 (most positive), along with key emotional indicators."},
        ...
        return error_response(error, "/analyze_sentiment")
...
async def setup_clients():
    ...
    USE_BRAINTRUST_PROXY = os.getenv("USE_BRAINTRUST_PROXY", "").lower() == "true"
    ...
    search_credential = AzureKeyCredential(search_key) if search_key else azure_credential
    ...
    if OPENAI_HOST.startswith("azure"):
        ...
        if api_key := os.getenv("AZURE_OPENAI_API_KEY_OVERRIDE"):
        ...
        elif USE_BRAINTRUST_PROXY:
            current_app.logger.info("Using Braintrust proxy for Azure OpenAI client")
            ...
            braintrust.init_logger("PythonAgent")
        else:
