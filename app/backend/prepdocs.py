...
def setup_blob_manager(
    ...
    storage_creds={"account_name": storage_account_name, "account_key":storage_key}
    return BlobManager(
...
synonym_map = {
    "name": "error-synonyms",
    "format": "solr",
    "synonyms": "error, critical issue, non-critical issue, issue, bug"
}

# Create or update synonym map in Azure AI Search
async def setup_synonym_map():
    search_endpoint = f"https://{os.environ['AZURE_SEARCH_SERVICE']}.search.windows.net"
    if isinstance(search_info.credential, AzureKeyCredential):
        headers = {
            "Content-Type": "application/json",
            "api-key": search_info.credential.key
        }
    else:
        token = await search_info.credential.get_token("https://search.azure.com/.default")
        headers = {
            "Content-Type": "application/json",
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"{search_endpoint}/synonymmaps?api-version=2024-07-01",
            headers=headers,
            json=synonym_map
        ) as response:
            if response.status in (200, 201):
                logger.info("Successfully created/updated synonym map")
            else:
                logger.error(f"Failed to create synonym map: {response.status}")
...
if __name__ == "__main__":
    ...
    # Execute synonym map creation
    loop.run_until_complete(setup_synonym_map())
