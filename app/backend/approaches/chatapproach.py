...
class ChatApproach(Approach, ABC):
    ...
    <<How can I use floating holidays?>>
    ...
    async def get_search_query(self, chat_completion: ChatCompletion, user_query: str):
            ...
            for tool in response_message.tool_calls:
                ...
                if function.name == "search_sources":
                    ...
                    if lat != self.NO_RESPONSE and lng != self.NO_RESPONSE and location != self.NO_RESPONSE:
                        ...
                        agent_response = f"Current weather in {location}: {temp}°F with winds at {wind} mph and {precip} inches of precipitation."                            
                    else:
                        agent_response = "Please provide a valid location."
                elif function.name == "get_joke":
                    joke_data = await self.get_joke()
                    agent_response = f"Here's a joke:\n\n{joke_data['joke']}"
                return f"Agent: {agent_response}" if agent_response else query_text
